...
class FoldIteration(hk.Module):
  ...
  def __call__(self,
    ...
    #Here all atom positions are mapped
...
class StructureModule(hk.Module):
  ...
  def loss(self, value, batch):
    ...
    #Harmonic potential - for finetuning
    harmonic_bond_loss(ret, batch, value, self.config)
...
def backbone_loss(ret, batch, value, config):
  ...
  fape_loss = fape_loss_fn(rigid_trajectory, gt_rigid, backbone_mask_protein,
...
def sidechain_loss(batch, value, config):
  ...
  #Multiply the gt_exists mask with the protein mask to only select these frames
  ...
  flat_positions_mask = jnp.reshape(batch['atom14_gt_exists'], [-1])
  ...
  fape = all_atom.frame_aligned_point_error(
      ...
      frames_mask=flat_frames_mask, #Here, all ligand atoms are excluded - there are no frames for these
...
def harmonic_bond_loss(ret, batch, value, config):
  """Computes loss for ligand bonds.

  Jumper et al. (2021) Suppl. Alg. 27 "torsionAngleLoss"

  Args:
    ret: Dictionary to write outputs into, needs to contain 'loss'.
    batch: Batch, needs to contain 'ligand feats' and 'ligand_bond_mask'.
    value: Dictionary containing structure module output, needs to contain
      value['final_atom14_positions']
    config: Configuration of loss, should contain 'harmonic_bond_weight'
  """

  pred_pos = value['final_atom14_positions'][:,1,:]
  pred_dmat = jnp.sqrt(1e-10 + jnp.sum((pred_pos[:, None] - pred_pos[None, :])**2, axis=-1))
  bond_lengths = batch['ligand_feats'][:,:,7]
  bond_mask = batch['ligand_bond_mask'] #LxL
  #Calculate deviation
  bond_diff = (pred_dmat-bond_lengths)**2
  #clip
  bond_diff = jnp.clip(bond_diff, 0, 10)
  #Average bond pos
  bond_dist_err = jnp.sum(bond_diff*bond_mask) / jnp.sum(bond_mask + 1e-8)

  ret['harmonic_bond_loss'] = bond_dist_err
  ret['loss'] += config.harmonic_bond_weight*bond_dist_err
