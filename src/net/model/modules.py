...
class Umol(hk.Module):
  """Umol model with recycling.
  """

  def __init__(self, config, name='Umol'):
    super().__init__(name=name)
    self.config = config
  ...
  def __call__(
    ...
    Returns:
      When compute_loss is True:
        a tuple of loss and output of FoldIteration.
      When compute_loss is False:
        just output of FoldIteration.

      The output of FoldIteration is a nested dictionary containing
      predictions from the various heads.
    """

    impl = FoldIteration(self.config, self.global_config)
...
class PredictedLDDTHead(hk.Module):
  ...
  def loss(self, value, batch):
    ...
    #Normalise the lddt
    #Protein lddt
    protein_lddt = jnp.sum(lddt_ca * batch['protein_mask']) / (jnp.sum(batch['protein_mask']) + 1e-8)
    #Ligand lddt
    ligand_lddt = jnp.sum(lddt_ca * batch['ligand_mask']) / (jnp.sum(batch['ligand_mask']) + 1e-8)

    #Ligand distance error
    pred_points = pred_all_atom_pos[:, 1, :]
    true_points=true_all_atom_pos[:, 1, :]
    dmat_predicted = jnp.sqrt(1e-10 + jnp.sum((pred_points[:, None] - pred_points[None, :])**2, axis=-1))
    dmat_true = jnp.sqrt(1e-10 + jnp.sum((true_points[:, None] - true_points[None, :])**2, axis=-1))
    dist_l1 = jnp.abs(dmat_true - dmat_predicted)
    ligand_sq_mask = batch['ligand_mask'][:,None]*batch['ligand_mask'][None,:]
    ligand_dist_err = jnp.sum(dist_l1*ligand_sq_mask) / jnp.sum(ligand_sq_mask + 1e-8)

    output = {'loss': loss, 'protein_lddt': protein_lddt, 'ligand_lddt': ligand_lddt, 'ligand_dist_err': ligand_dist_err}
    return output
...
class EmbeddingsAndEvoformer(hk.Module):
  ...
  def __call__(self, batch, is_training, safe_key=None):
    ...
    #Embed amino acids and ligand atom types into 2D
    ...
    #Add an embedding of the ligand bond types, bond lengths and protein target positions
    pair_activations += common_modules.Linear(
        c.pair_channel, name='pair_activiations')(
            batch['ligand_feats'])
    ...
    # Extra MSA Stack.
    # First run everything through this - then run with the main MSA
