...
def to_pdb(prot: Protein) -> str:
  ...
  ligand_atoms = prot.ligand_atoms
  li=0
  ...
  for i in range(aatype.shape[0]):
    ...
    #Check if not amino acid --> map atom only and chain B
    if aatype[i]<0: #The pad pos
        continue

    #Write ligand atoms
    # To do: add the smile bond order
    if aatype[i]>20:
        res_name_3='   '
        atom_name = ligand_atoms[li]
        chain_id = 'B'
        record_type = 'HETATM'
        pos = atom_positions[i,1]
        name = atom_name if len(atom_name) == 4 else f' {atom_name}'
        alt_loc = ''
        insertion_code = ''
        occupancy = 1.00
        b_factor = b_factors[i,1]
...
def from_prediction(features: FeatureDict, result: ModelOutput,
  ...
  return Protein(
      ...
      b_factors=b_factors,
      ligand_atoms=features['ligand_atoms'])
